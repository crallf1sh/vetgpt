...
function validateForm(data) {
  ...
  // Require either symptoms OR notes
  if (data.symptoms.length === 0 && !data.notes.trim()) {
    errors.push("Please select at least one symptom or describe the issue in Additional Notes.");
  }
...
function renderResult(data, petName = "Your pet") {
  const urgencyText = (data.urgency || "").toLowerCase();
  const urgencyClass =
    urgencyText.includes("emergency") || urgencyText.includes("soon")
      ? "urgent"
      : "monitor";
